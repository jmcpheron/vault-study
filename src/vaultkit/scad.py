...
def _scad_header() -> str:
    """Common preamble: $fn for smooth circles, the Y-up→Z-up rotation."""
    return (
        "// Generated by vaultkit.scad — do not hand-edit.\n"
        "// STEP files are Y-up; OpenSCAD is Z-up. The outer rotate()\n"
        "// makes the SCAD source read with Z pointing up the door axis.\n"
        "$fn = 64;\n\n"
        "module y_up_to_z_up() {\n"
...
def cam_sweep_template(
    ...
    *,
    ...
    bcd_r = params.SPUR_BCD_MM / 2  # 36 mm — use measured BCD for visual layout
    ...
    # Hub + base — stationary, drawn first so they sit at the bottom of the
    # z-buffer.
    for role in ("hub", "base"):
        for stl in by_role.get(role, []):
            lines.append(f"// {role}")
            lines.append(f'y_up_to_z_up() import("{stl}");')
    lines.append("")

    # Ring — single instance, oscillates about Z.
    for stl in by_role.get("ring", []):
        lines.append("// ring — oscillates about Z")
        lines.append("rotate([0, 0, theta])")
        lines.append(f'    y_up_to_z_up() import("{stl}");')
    lines.append("")

    # 12 spurs — each at angle phi_i, counter-rotating about its own Z.
    # Each spur is positioned at (BCD/2 * cos(phi_i), BCD/2 * sin(phi_i)).
    spur_stls = by_role.get("spur", [])
    if spur_stls:
        spur_stl = spur_stls[0]  # one unique STL, instanced 12×
        lines.append(f"// 12 spurs at {bcd_r} mm radius, 30° apart, counter-rotating")
        lines.append("for (i = [0 : 11]) {")
        lines.append("    phi = i * 30;")
        lines.append(f"    translate([{bcd_r} * cos(phi), {bcd_r} * sin(phi), 0])")
        lines.append("        rotate([0, 0, spur_angle])")
        lines.append(f'            y_up_to_z_up() import("{spur_stl}");')
        lines.append("}")
        lines.append("")
    ...
    if rack_stls or pin_stls:
        ...
        lines.append("for (i = [0 : 11]) {")
        lines.append("    phi = i * 30;")
        # Position the assembly at the spur location, oriented so radial
        # outward is +X in the local frame.
        lines.append("    rotate([0, 0, phi])")
        lines.append(f"        translate([{bcd_r} + rack_dr, 0, 0])")
        lines.append("    {")
        if rack_stls:
            lines.append(f'        y_up_to_z_up() import("{rack_stls[0]}");')
        if pin_stls:
            lines.append(f'        y_up_to_z_up() import("{pin_stls[0]}");')
        lines.append("    }")
        lines.append("}")
...
def exploded_template(
    ...
    *,
    ...
    # Ring — lift along +Z as it explodes.
    for stl in by_role.get("ring", []):
        lines.append("// ring — lifts along +Z")
        lines.append("translate([0, 0, progress * max_offset * 0.5])")
        lines.append(f'    y_up_to_z_up() import("{stl}");')
    lines.append("")

    # 12 spurs — fan outward radially.
    spur_stls = by_role.get("spur", [])
    if spur_stls:
        ...
        lines.append(
            ...
            f" ({bcd_r} + progress * max_offset) * sin(phi), 0])"
        )
        lines.append(f'        y_up_to_z_up() import("{spur_stls[0]}");')
        lines.append("}")
        lines.append("")
    ...
    if rack_stls or pin_stls:
        ...
        lines.append("    rotate([0, 0, phi])")
        lines.append(
            f"        translate([{bcd_r} + progress * max_offset * 1.5, 0, 0])"
        )
        lines.append("    {")
        if rack_stls:
            lines.append(f'        y_up_to_z_up() import("{rack_stls[0]}");')
